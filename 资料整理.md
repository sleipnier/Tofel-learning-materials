...
7. stone : **opal** 蛋白石 **emerald** 祖母绿 **sapphire**蓝宝石
...
2. **许多，大量的** lots of = a great number of(不可数) = tons of = numerous = vast = a mass of = plentiful = plenty of = a great deal of = a great amount of(可数) = immense = substantial = considerable = substantial = a host of = astronomical = tremendous（巨大的） = a bulk of 
...
5. **然而** However = nevertheless(*adv*) = nonetheless(*adv*)
6. **人们，人类** people = individuals = mankinds = 
7. **重要的** important = essential = crucial = significant = critical = vital =
8. **突然的**  abrupt = sudden = precipitate = unexpected
9. **惊讶的** startling = surprise = striking = amazing = 
10. **相同的** common（常见的） = same 
11. **不同的** differential = different
12. **思考** think = contemplate = ponder
13. **假定，假设** assume = postulate = posit
14. **开始** begin = start = genesis(n.)  = commence（n.） = inception(n.)
15. **邻居** neighborhood = nearby community
16. **支持** support = back up = endorse(签名、赞同)
17. **帮助** help = assist sb in doing = do sb a favor = aid sb to do 
18. **反对** oppose = rebute = dispute
19. **出现** arise = emerge = appear
20. **促进** enhance = improve = facilitate = promote
21. **上升** rise = pop up
22. **下降** decline = abate(减弱) = detract = go down = 
23. **加强** intensify =  escalate = enhance = 
24. **表明,表现** show = reflect = indicate = manifest =  reveal
25. **昂贵的** expensive = costly = unafffordable = prohibitive
26. **便宜的** cheap = cost-effective = inexpensive = ten-cent
27. **快速的，敏捷的** quick = swift = 
28. **迟缓的，缓慢的** slow = sluggish
29. **静止的** quiescent = resting = still = 
30. **贫穷的** poor = indigent
31. **大厦** edifice = mansion = skycraper 
32. **破坏，灾难** damage = havoc = catastrophy = disaster = tregedy = plague(一般指瘟疫) = destruction = anomie（社会道德沦丧）= calamity
33. **连续的** consecutive = continue =
34. **首要的、主要的** imperative = priority = main = chief = staple = prime = 
35. **次要的** secondary = subordinate = lesser = minor = 
36. **奢华的，豪华的** sumptuous = luxury = magnificent
37. **与……有关** pertain（v.） = considering = about =  regarding
38. **抓住** clutch at = grasp = grip = capture
39. **开始，开端** beginning = genesis = origin
40. **优点** advantage = pros = merits
41. **缺点** disadvantage = cons = **adverse affect** = deficiency = demerits = deficient（缺乏、不足）
42. **影响** impacts = influence = affect = 
43. **教学 教育** teaching = instruction
44. **忽视** neglect = ignore = overlook
45. **前提** premise = precondition = prerequisite
46. **基础** basic  = bedrock = fundament
47. **保护** protect = conserve = preserve
48. **个性** individuality = personality(一般指人的) = character = distinction(一般指差异) = trait(特性、特点)
49. **相对的，相比较而言的** relative = comparative
50. **厌恶，憎恨** hate = abhor = abominate
51. **勇敢的** brave = courageous = bold（也指大胆的，冒失的）
52. **入侵，侵入** invasion = intrusion =
53. **丰富的** affluent = ample（丰裕的）
54. **流言蜚语** gossip = rumor
55. **主旨，主题** heading = motif = thesis
56. **污染** contamination = pollution 
57. **有礼貌的** courteous = polite
58. **名声，声望** prestige = reputation = renown
59. **传播** spread = propagate = transmit （transmit disease）= transit(一般指运送)
60. **流行的** prevalent = popular
61. **青年，学生** students = teenagers = juvenile(n.&adj.) = adolescent =youngsters
62. **辩论、争论** dispute = debate = argue = 
63. **满意** satisfy = gratify（使满意）
64. **总共** total = gross 
65. **基本的** basic = rudimentary = essential = elementary = fundamental = primary
66. **抑制，镇压** repress = suppress = restrain = 
67. **伸出、探出** protrude = reach out = stick out = extend 
68. **脆弱的、易碎的**  fragile = frail = delicate
69. **先驱** precursor = pioneer
70. **几乎** almost = nearly = virtually
71. **方法、途径** a way to do = an approach to doing = an avenue for doing 
72. **外向的** outgoing = extrovert
73. **内向的** introvert = inward
74. **仪器装置，设备** apparatus = device = equipment = facility
75. **通知** inform = apprise
76. **协议、条约** pact = terms
77. **原则、信条** tenet = gospel = principal
78. **标志 象征** emblem = symbol
79. **暂时的** temperary = transient
80. **温和的** temperate = mild = moderate
